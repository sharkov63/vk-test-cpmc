...
namespace cpmc {
    ...
    bool Tokenizer::isAlphabetic(char c) {
        return ('a' <= c && c <= 'z') || ('A' <= c && c <= 'Z');
    }

    bool Tokenizer::isWordSymbol(char c) {
        return c == '_' || isAlphabetic(c) || isDigit(c);
    }
    ...
    Token Tokenizer::nextNumericLiteralToken() {
        ...
        if (s[i - 1] == '.') {
            throw InvalidTokenException(token,
                                        "A floating-point number literal ends with a dot.");
        }
        if (dotCount == 0) {
